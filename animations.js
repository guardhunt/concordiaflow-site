// ------------------------------------------------------------
// Animation functionality (logo flaps + ripple overlay)
...
function scheduleLogoFlaps() {
  ...
  const scheduleNext = () => {
    ...
    nextTimerId = window.setTimeout(() => {
      ...
      rightWing.addEventListener('animationend', () => endBurst(token), { once: true });
    ...
    if (currentInsts.length > 0) {
      currentInsts.forEach((inst) => {
        ...
        inst.ringGroups.forEach((rg, ri) => {
          const op =
            elapsed >= 0 && elapsed < RIPPLE_TOTAL_DURATION ? getRingOpacity(ri, elapsed) : 0;
